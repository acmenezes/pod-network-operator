	...
	for _, r.podNetworkConfig = range r.podNetworkConfigList.Items {
		...
		for _, pod := range podList.Items {
			...
			if podNetworkConfig.Status.Phase != podnetworkv1alpha1.PodNetworkConfigConfigured {
				...
				for _, p := range podNetworkConfig.Status.PodNetworkConfigurations {
				...
				if isPodNamePresent == false {
					...
					podNetworkConfig.Status.PodNetworkConfigurations = append(podNetworkConfig.Status.PodNetworkConfigurations, configStatus)
...
func (r *PodNetworkConfigReconciler) listPodsWithMatchingLabels(podNetworkConfig podnetworkv1alpha1.PodNetworkConfig) (*corev1.PodList, error) {
